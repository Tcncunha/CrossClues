...
const PORT = 4002; const URL = `http://localhost:${PORT}`;
...
function emitAck(s,e){return new Promise(res=>s.emit(e,(r)=>res(r)));}
async function run(){
  const sp=spawn('node',['server-diag.js'],{cwd:PROJECT,env:{...process.env,NODE_ENV:'development',SUPABASE_URL:'http://localhost:29999',SUPABASE_KEY:'t',ADMIN_SECRET:'s',PORT:String(PORT)},stdio:'pipe'});
  ...
  console.log('calling start-game ONCE (no payload)');
